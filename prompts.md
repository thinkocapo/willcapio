2024.12.19 Make a migration plan for migrating this project to a Nextjs + FastAPI app which can deployed to Vercel.
...
SEO-Critical, Dynamic Pages: For pages that need to be indexed by search engines but have content that's too dynamic to be pre-rendered at build time, like a product page with real-time stock availability, SSR ensures search engine crawlers receive fully-rendered, up-to-date HTML.
